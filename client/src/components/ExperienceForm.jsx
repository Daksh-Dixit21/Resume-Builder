...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const handleEnhance = async (index) => {
        ...
        try {
            const response = await api.post('/api/ai/enhance-job-desc', { prompt: data[index].description });
            const enhancedDescription = response.data.data;
                    ...
                    {data.map((experience, index) => (
                        <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                            ...
                            <label className='flex items-center gap-2'>
                                <input type="checkbox" checked={experience.is_current || false} onChange={(e) => { updateExperience(index, "is_current", e.target.checked ? true : false); }} className='rounded border-gray-300 text-blue-600 focus:rind-blue-500' />
                            ...
                            <div className='space-y-2'>
                                <div className='flex items-center justify-between'>
                                    ...
                                    <button
                                        ...
                                        {isGenerating === index ? 'Generating...' : 'Enhance with AI'}
                                    </button>
                                 </div>
